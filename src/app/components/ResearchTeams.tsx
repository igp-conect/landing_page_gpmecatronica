                        ...
                        >
                            <div className="tw:group tw:bg-white tw:p-10 tw:shadow-lg tw:h-full tw:relative tw:transition-all tw:duration-300 hover:tw:shadow-xl">
                                <div className="tw:w-12 tw:h-12 tw:relative tw:mb-12">
                                    <i className={`${team.icon} tw:text-5xl tw:text-gray-700 tw:relative tw:z-10 tw:transition-colors tw:duration-300 tw:group-hover:text-gray-900`} />
                                ...
                                <div className="tw:mt-4">
                                    <strong className="tw:text-sm tw:text-gray-800">
                                        Áreas de Foco:
                                    </strong>
                                    <ul className="tw:mt-2 tw:pl-5 tw:list-disc">
                                        {team.focus.map((item) => (
                                            <li
                                                key={item}
                                                className="tw:text-sm tw:leading-6 tw:text-gray-600"
                                            >
                                                {item}
                                            </li>
                                        ))}
                                    ...
                                    <div className="tw:mt-3">
                                        ...
                                        <div className="tw:mt-2 tw:flex tw:flex-wrap tw:gap-1.5">
                                            {team.members.map((member) => (
                                                <span
                                                    key={member.name}
                                                    className="tw:text-xs tw:px-2.5 tw:py-1 tw:bg-accent/10 tw:text-accent tw:rounded-full"
                                                >
                                                    {member.name}
                                                </span>
                                            ))}
